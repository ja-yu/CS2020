...
4.通過郵件客戶端直接連接到伺服器上，直接對郵件進行操作，郵件保留在伺服器上
...
3.
4.常用用途是單一登入，用戶可以在多個服務中使用同一個密碼，通常用於公司內部網路站的登錄中
5.LDAP目錄

SAMBA
1.SAMBA運作在Application layer
2.Unix環境下實作Windows環境的SMB/CIFS協定功能
3.可以讓Unix環境與Windows環境共享彼此的資源

SNMP
1.SNMP英文全名為Simple Network Management Protocol
2.SNMP運作在Application layer
3.是Internet標準協議，用於收集和組織有關IP網路上被管理的設備的資訊，並修改資訊以更改設備行為。
...
3.用來進行實時數據通信的網絡協議
4.使用TCP協定，保證影片質量
5.主要用來在Flash/AIR平台和支持RTMP協議的串流媒體與伺服器之間進行音視頻和數據通信。
Adobe的私有協議,未完全公開

RTSP
1.RTSP英文全名為Real Time Streaming Protocol
2.RTSP運作在Application layer
3.是一種網路應用協定，專為娛樂和通信系統的使用，以控制串流媒體伺服器。
4.RTSP媒體伺服器支援流式音訊和視訊的傳送，在客戶端的媒體播放器可以邊下載邊播放
5.使用的是UDP
共有協議，並有專門機構做維護。
...
2.MAC位址用來確認網路裝置位置的位址，每個網卡會有一個唯一的MAC位址。
3.用在Data Link layer可辨識傳送資料的來源及目的地
```
```
HTTP
